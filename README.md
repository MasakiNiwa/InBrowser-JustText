...
- **日本語 / English** — 端末の言語で自動的に切り替わり、設定からいつでも変更できる
...
src/
  ...
  i18n/               表示言語の切り替えと辞書
  io/                 ファイルの読み込み・保存・共有の受け取り・クリップボード
...
1. `src/i18n/locales/ja.js` を写して `src/i18n/locales/xx.js` を作り、値を訳す（キーはそのまま）
2. `src/i18n/index.js` の `LOCALES` と `CATALOGS` に追加する
3. `sw.js` の `APP_SHELL` にファイルを追加し、`VERSION` を上げる

`npm test` が、日本語の辞書とキーが揃っているか・`{name}` の差し込みがずれていないか・
画面に書かれた文言と辞書が一致しているかを確認します。
