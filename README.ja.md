...
- **タブと記号の行** — ソフトキーボードに無いタブや、奥に隠れている記号をキーボードを閉じずに入力できる
- **自動保存と復元** — 編集中の内容を端末内に控え、次の起動時に復元するか尋ねる
- **ツール** — JSON の整形・最小化・検証・キーの並べ替え、行の複製・削除・移動・並べ替え・重複削除、行末の空白削除など
...
ソフトキーボードにはタブキーが無く、JSON や設定ファイルでよく使う記号も
たいてい 2 階層ほど奥にあります。ステータスバーの上の行から、その両方を 1 タップで
入力できます。フォーカスを奪わないので、キーボードは開いたままです。

タブはハードウェアキーボードの Tab キーと同じく、インデントの設定に従います。
不要なら設定から非表示にできます。
...
ファイル名の横の点が状態を表します。未保存の変更があると表示され、
その変更が控えられていないとき（内容が大きすぎる、保存先が使えない）は
**オレンジ**になります。

- 控えが残るのは**未保存の変更があるあいだだけ**です。保存が済むと消えます
- 控えは端末の中だけに置かれ、外には出ません
- 約 **4 MB** を超える内容は控えを取れません。その場合は一度だけお知らせします。
  約 2 MB を超えると元のバイト列は控えから外れるため、
  復元後は文字コードを指定して開き直せません
- タブごと、および共有から開いたファイルごとに別々の控えを持ちます。
  まだ尋ねていない控えを上書きすることはありません。
  他のタブが使用中の控えは表示しません（破棄するとそのタブの唯一の控えが消えるため）
- 残っている控えは一覧で表示します。ファイル名・最終更新時刻・内容の冒頭が並び、
  復元する・1 件だけ破棄する・すべて破棄する・あとにする、から選べます。
  **ダイアログを閉じるまで実際には削除しません**。誤って ✕ を押しても、
  もう一度押せば元に戻せます
- 同じ一覧はツールメニューの **この端末に残っている未保存の内容** からも開けます。
  再起動しなくても確認できます
- 保持期間を過ぎた控えは、次回起動時に**一覧を出す前に**削除します。
  そのため、一覧に出たものは「あとで」を選べば確実に残ります。
  他のタブが使用中の控えは、期間を過ぎていても削除しません
- 保存できない環境（プライベートモードや容量不足）では黙って諦めます。
  あくまで保険なので、確実に残したいときは保存してください

<p><img src="docs/screenshot-drafts.png" alt="残っている下書きの一覧" width="300"></p>

### 控えを残したくない場合

設定に専用の項目があります。控えを取らない、保持期間を変える（1 / 7 / 30 / 90 日）、
この端末の控えをすべて削除する、が選べます。控えを取らない設定にすると
ファイル名の横の点がその旨を示し、保存先には何も書き込みません。

<p><img src="docs/screenshot-privacy.png" alt="未保存の内容に関する設定" width="300"></p>

## JSON とデータの安全性

JSON ツールはブラウザ内蔵のパーサを通します。このパーサは 2 つのことを黙って変えます。
扱えない大きさの数値は丸められ、同じオブジェクトにキーが 2 回あると最後のものだけが残ります。
設定ファイルをそのように書き換えてしまうのは、何もしないより悪いことです。
そのため、該当する場合は**変換を行わず**、原因を示してカーソルをその位置へ移動します。
**JSON を検証**でも同じ内容を報告するので、問題になる前に気づけます。

`-0` も同様です。JavaScript は `0` と区別できますが、書き出すと `0` になるためです。

`__proto__` は JSON では普通のキーなので、そのまま保持します。
